...
2. Tunjukkan bahwa matriks $A$ dan $\begin{bmatrix}A & AB\end{bmatrix}$ memiliki column space yang sama.
3. Sebuah matriks $n \times n$ memiliki column space yang sama dengan $\mathbb{R}^n$ jika dan hanya jika matriks tersebut _____.
...
2. Kolom $AB$ adalah kombinasi linear dari kolom $A$, sehingga column space $\begin{bmatrix}A & AB\end{bmatrix}$ telah ada di $C(A)$.
3. Invertible.